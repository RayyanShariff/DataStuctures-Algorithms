...
public class DLinkedList {
	...
	public void insertionSort() {
		...
		//iterates thru the nodes until currentNode is equal to null
		while(currentNode != null) {
			
			//goes to the next node
			Node next = currentNode.nextNode;
			
			//updating node value when next node are not sorted
			while((next != null) && (next.prevNode != null)&&(next.value < next.prevNode.value)) {
				// iterates until if currentNode is pointing to the next node and next node is pointing to a previous node
				// so that the next nodes can be compared then switched if it is not sorted
				int val = next.value;
				next.value = next.prevNode.value;
				next.prevNode.value = val;
				next = next.prevNode;
			}
			//goes to the next node
