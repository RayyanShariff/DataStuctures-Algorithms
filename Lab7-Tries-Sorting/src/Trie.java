...
public class Trie {
	...
	private class TrieNode { // This is the class that represents a node in the trie
		...
		public int countAllWords() {
			//numberOfWords is the counter
			int numberOfWords = 0;
						
			//checks if the word is valid and if its valid, then it increments the counter
			if(isValidEnd) {
				numberOfWords++;
			}
			
			//recursively checks for subnode if all words isValidEnd == True 
			for(int i=0; i<subnodes.length; i++) {
				if(subnodes[i] != null) {
