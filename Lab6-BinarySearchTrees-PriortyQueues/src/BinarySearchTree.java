public class BinarySearchTree {
	...
	private class BSTNode { // private class to hold a tree node
		...
		// Part 1: complete
		/**
		 * Method that performs in-order traversal by printing out the numbers in a tree, in the correct order
		 */
		public void inOrderTraversalPrint() {
			if (leftChild != null) 
				// recurse down the left sub tree until its equal to null
				leftChild.inOrderTraversalPrint();
			
			// when its equal to null, meaning it reached the last child then it prints out that value	
			System.out.println(this.value);
			
			if(rightChild != null) 
				// recurse down the right sub tree until its equal to null
		...
		// Part 2: complete
		/**Method that performs in-order traversal and it adds it to the tail of doubly linked list, so in the end its a doubly linked list in-order
		 * @param dl doubly linked list from the previous inOrderTraversal method
		 */
	...
	// Part 1
	/**
	 * A method that checks if the binary tree is empty and if its not empty then it calls the inOrderTraversalPrint() method
	 */
	public void inOrderTraversalPrint() {
		if (rootNode != null)
	...
	// Part 2: complete
	/**A method that checks if the binary tree is empty and if its not empty then it calls the inOrderTraversalPrint() method
	 * @return a doubly linked list in order
	 */
